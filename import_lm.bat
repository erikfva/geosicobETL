...
%PATH_OGR2OGR%ogr2ogr -f "PostgreSQL" PG:"host=%pghost% user=%pguser% password=%pgpass% port=%pgport% dbname=%pgdb% ACTIVE_SCHEMA=%pgschema%" %PATH_GDB% -lco GEOMETRY_NAME=%pggeom% -lco FID=%fnPrefix%_id -overwrite -progress --config PG_USE_COPY YES -nln %newlyrname% -sql "%sql% FROM \"%lyrname%\""
