...
REM ********************************************************************************
psql -h %pghost% -p %pgport% -c "DROP TABLE IF EXISTS %pgschema%.pop_vigente; CREATE TABLE %pgschema%.pop_vigente AS SELECT * FROM %pgschema%.pop WHERE trim(est_der) <> 'NO VIGENTE';" postgresql://%pguser%:%pgpass%@%pghost%/%pgdb%
REM ** Adicionando �ndices **
psql -h %pghost% -p %pgport% -c "ALTER TABLE %pgschema%.pop_vigente ADD PRIMARY KEY (%fnPrefix%_id);" postgresql://%pguser%:%pgpass%@%pghost%/%pgdb%
