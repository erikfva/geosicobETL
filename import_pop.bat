...
(echo CREATE INDEX pop_idx_est ON %pgschema%.pop USING btree (est_der^^^);) | psql -h %pghost% -p %pgport% -U %pguser% -d %pgdb%
REM ********************************************************************************
REM **Creando capa de POP vigentes (por razones de performance del sistema)**
REM ********************************************************************************
(echo DROP TABLE IF EXISTS %pgschema%.pop_vigente; CREATE TABLE %pgschema%.pop_vigente AS SELECT * FROM %pgschema%.pop WHERE trim(est_der^^^) = 'VIGENTE';)  | psql -h %pghost% -p %pgport% -U %pguser% -d %pgdb%
REM ** Adicionando �ndices **
