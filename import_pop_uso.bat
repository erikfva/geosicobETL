...
REM ********************************************************************************
psql -h %pghost% -p %pgport% -c "DROP TABLE IF EXISTS %pgschema%.pop_uso_vigente; CREATE TABLE %pgschema%.pop_uso_vigente AS SELECT * FROM %pgschema%.pop_uso WHERE trim(est_der) = 'VIGENTE';" postgresql://%pguser%:%pgpass%@%pghost%/%pgdb%
REM ** Adicionando indices **
psql -h %pghost% -p %pgport% -c "ALTER TABLE %pgschema%.pop_uso_vigente ADD PRIMARY KEY (gv_id);" postgresql://%pguser%:%pgpass%@%pghost%/%pgdb%
